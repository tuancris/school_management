...
class SchoolAPI(odoo.http.Controller):
    ...
    @odoo.http.route(['/school/<odoo_15>/<id>'], type='http', auth="none", sitemap=False, cors='*', csrf=False)
    def pet_handler(self, odoo_15, id, **kw):
        model_name = "school.information"
        try:
            registry = odoo.modules.registry.Registry(odoo_15)
